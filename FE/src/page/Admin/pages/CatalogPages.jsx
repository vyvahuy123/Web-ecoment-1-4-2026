      ...
      <div className="card">
        <div className="card__head">
          <span className="card__title">
            Danh mục sản phẩm ({loading ? "..." : cats.length})
          </span>
        </div>
              ...
              {loading
                ...
                : cats.map((c) => (
                    <tr key={c.id}>
                      ...
                      <td style={{ color: "var(--g4)", fontSize: 13 }}>
                        {c.description ?? "—"}
                      </td>
                      <td>
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      await OrderService.updateStatus(
        statusTarget.id,
        STATUS_MAP_TO_INT[newStatus] ?? newStatus,
      );
            ...
            <tbody>
              {loading
                ...
                : orders.map((o) => {
                    ...
                    const pay = PAY_STATUS_MAP[o.paymentStatus] ?? {
                      label: o.paymentStatus,
                      cls: "badge-inactive",
                    };
                    const canChangeStatus =
                      !!STATUS_TRANSITIONS[o.status]?.length;
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1)
              ...
              .map((p, i) =>
                p === "..." ? (
                  <span
                    key={i}
                    style={{ padding: "0 4px", color: "var(--g3)" }}
                  >
                    …
                  </span>
                ) : (
                  <button
                    key={p}
                    className={`btn btn-sm${page === p ? " btn-dark" : ""}`}
                    onClick={() => setPage(p)}
                  >
                    {p}
                  </button>
        ...
        <div className="modal-overlay" onClick={() => setDetailOrder(null)}>
          ...
          >
            ...
            {detailOrder._loading ? (
              <div
                style={{ padding: 32, textAlign: "center", color: "var(--g3)" }}
              >
                Đang tải...
              </div>
            ) : (
              ...
              >
                ...
                >
                  <InfoBlock label="Trạng thái">
                    <span
                      className={`badge ${ORDER_STATUS_MAP[detailOrder.status]?.cls ?? "badge-inactive"}`}
                    >
                  ...
                  <InfoBlock label="Thanh toán">
                    <span
                      className={`badge ${PAY_STATUS_MAP[detailOrder.paymentStatus]?.cls ?? "badge-inactive"}`}
                    >
                      {PAY_STATUS_MAP[detailOrder.paymentStatus]?.label ??
                    ...
                    >
                      {PAY_METHOD_MAP[detailOrder.paymentMethod] ??
                        detailOrder.paymentMethod}
                    </span>
                  </InfoBlock>
                  <InfoBlock label="Ngày đặt">
                    {fmtDatetime(detailOrder.createdAt)}
                  </InfoBlock>
                  <InfoBlock label="Ngày TT">
                    {fmtDatetime(detailOrder.paidAt)}
                  </InfoBlock>
                  <InfoBlock label="Ngày giao">
                    {fmtDatetime(detailOrder.shippedAt)}
                  </InfoBlock>
                  <InfoBlock label="Ngày nhận">
                    {fmtDatetime(detailOrder.deliveredAt)}
                  </InfoBlock>
                </div>
                ...
                >
                  ...
                  <div style={{ fontSize: 14, fontWeight: 600 }}>
                    {detailOrder.shippingFullName} — {detailOrder.shippingPhone}
                  </div>
                  <div
                    style={{ fontSize: 13, color: "var(--g4)", marginTop: 2 }}
                  >
                    {detailOrder.shippingStreet}, {detailOrder.shippingWard},{" "}
                    {detailOrder.shippingDistrict},{" "}
                    {detailOrder.shippingProvince}
                  </div>
                  {detailOrder.note && (
                    <div
                      style={{ fontSize: 12, color: "var(--g3)", marginTop: 4 }}
                    >
                ...
                >
                  ...
                  <SumRow
                    ...
                    value={fmt(detailOrder.shippingFee)}
                  />
                  {detailOrder.discountAmount > 0 && (
                    <SumRow
                      label={`Voucher (${detailOrder.voucherCode})`}
                      value={`-${fmt(detailOrder.discountAmount)}`}
                      highlight
                    />
                  )}
                  <SumRow
                    label="Tổng cộng"
                    value={fmt(detailOrder.totalAmount)}
                    bold
                  />
                </div>
              </div>
            ...
            <div className="modal-foot">
              ...
              {!detailOrder._loading &&
                detailOrder.status === "PendingCancellation" && (
                  <button
                    className="btn btn-sm btn-danger"
                    onClick={() => {
                      setDetailOrder(null);
                      setApproveTarget(detailOrder);
                    }}
                  >
                    Duyệt hủy đơn
                  </button>
        ...
        <div className="modal-overlay" onClick={() => setStatusTarget(null)}>
          ...
          >
            ...
            <div className="modal-body">
              ...
              <div
                style={{ fontSize: 13, color: "var(--g4)", marginBottom: 12 }}
              >
                Đơn:{" "}
                <strong>
                  #
                  {statusTarget.orderCode ??
                    statusTarget.id?.slice(0, 8).toUpperCase()}
                </strong>
              </div>
      ...
      {approveTarget && (
        <div className="modal-overlay" onClick={() => setApproveTarget(null)}>
          ...
          >
            ...
            <div className="modal-body">
              <p style={{ fontSize: 14, color: "#555" }}>
                Xác nhận duyệt yêu cầu hủy đơn{" "}
                <strong>
                  #
                  {approveTarget.orderCode ??
                    approveTarget.id?.slice(0, 8).toUpperCase()}
                </strong>
                ?
              </p>
  ...
  return (
    <div
      style={{
        ...
        color: highlight ? "var(--red-text, #e53)" : "inherit",
      }}
    >
      <span style={{ color: "var(--g4)", minWidth: 140, textAlign: "right" }}>
        {label}
      </span>
...
export function VouchersPage() {
  ...
  const filtered = withStatus.filter(
    (v) => activeFilter === "Tất cả" || v._status === activeFilter,
  );
    ...
    <div>
      <div className="page-filter">
        {VOUCHER_FILTERS.map((ft) => (
          <button
            key={ft}
            className={`filter-tab${activeFilter === ft ? " active" : ""}`}
              ...
              {loading
                ...
                : filtered.map((v) => (
                    <tr key={v.id ?? v.code}>
                      ...
                      <td>
                        {v.minOrderAmount > 0 ? fmt(v.minOrderAmount) : "—"}
                      </td>
                      <td>
