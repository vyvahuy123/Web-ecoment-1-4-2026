      ...
      <div className="card">
        <div className="card__head">
          <span className="card__title">
            Danh gia ({loading ? "..." : filtered.length})
          </span>
        </div>
        {error && (
          <div
                      ...
                      <td style={{ maxWidth: 200 }}>
                        {replyingId === r.id ? (
                          ...
                          <span
                            style={{
                              fontSize: 12,
                              color: r.adminReply ? "var(--g4)" : "#bbb",
                              fontStyle: r.adminReply ? "normal" : "italic",
                            }}
                          >
...
/* ── PAYMENTS ─────────────────────────────────────────────────── */
const PAY_FILTERS = ["Tất cả", "Chưa TT", "Đã TT", "Hoàn tiền", "Thất bại"];
const PAY_STATUS = {
  ...
  return (
    <div>
      ...
      <div className="card">
        <div className="card__head">
          <span className="card__title">
            Lịch sử thanh toán ({loading ? "..." : filtered.length})
          </span>
        </div>
        {error && (
          <div
                    ...
                    return (
                      <tr key={p.id}>
                        <td style={{ fontWeight: 500 }}>
                          #{p.transactionId ?? p.id?.slice(0, 8).toUpperCase()}
                        </td>
                        <td style={{ color: "var(--g4)", fontSize: 12 }}>
                          {p.orderCode ?? p.orderId?.slice(0, 8).toUpperCase()}
                        </td>
                        <td>
  ...
  return (
    <div>
      ...
      <div className="card">
        <div className="card__head">
          <span className="card__title">
            Thông báo ({loading ? "..." : filtered.length})
          </span>
        </div>
        {error && (
          <div
          ...
          {loading
            ? Array.from({ length: 4 }, (_, i) => (
                <div key={i} className="list-row">
                  <div
                    className="skeleton-line"
                    style={{ height: 14, borderRadius: 4, flex: 1 }}
                  />
                </div>
              ))
            : filtered.map((n) => {
                ...
                return (
                  <div
                    ...
                    style={{ opacity: n.isRead || n.read ? 0.65 : 1 }}
                  >
                    <span style={{ fontSize: 16, flexShrink: 0 }}>
                      {n.icon ?? "🔔"}
                    </span>
                    <div className="list-row__main">{n.message ?? n.text}</div>
                    <div className="list-row__time">
                      {n.createdAt
