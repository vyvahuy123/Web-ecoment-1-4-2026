...
const STATUS_LABEL = {
  0: "Chờ xác nhận", 1: "Đã xác nhận", 2: "Đang xử lý",
  3: "Đang vận chuyển", 4: "Đã giao", 5: "Đã huỷ",
};
const PAYMENT_LABEL = {
  0: "Thanh toán khi nhận hàng (COD)", 1: "Chuyển khoản ngân hàng",
  2: "VNPay", 3: "MoMo", 4: "ZaloPay",
};
  ...
  return (
    <div className="os-wrap">
      <div className="os-card">
        ...
        <p className="os-subtitle">Cảm ơn bạn đã mua hàng. Đơn hàng của bạn đang được xử lý.</p>
              ...
              <div className="os-items">
                {order.items.map((item) => (
                  <div key={item.id} className="os-item">
                    ...
                    <span className="os-item-price">{item.totalPrice.toLocaleString("vi-VN")}₫</span>
                  </div>
                ))}
              </div>
            )}
            <div className="os-totals">
              <div className="os-total-row"><span>Tạm tính</span><span>{order.subTotal?.toLocaleString("vi-VN")}₫</span></div>
              <div className="os-total-row"><span>Phí vận chuyển</span><span>{order.shippingFee?.toLocaleString("vi-VN")}₫</span></div>
              {order.discountAmount > 0 && (
                <div className="os-total-row os-discount"><span>Giảm giá</span><span>−{order.discountAmount?.toLocaleString("vi-VN")}₫</span></div>
              )}
              <div className="os-total-row os-grand-total"><span>Tổng cộng</span><span>{order.totalAmount?.toLocaleString("vi-VN")}₫</span></div>
            </div>
          </div>
        )}
        <div className="os-actions">
          <button className="os-btn-primary" onClick={() => router.push("/")}>Tiếp tục mua sắm</button>
          <button className="os-btn-ghost" onClick={() => router.push("/orders")}>Xem đơn hàng của tôi</button>
        </div>
      </div>
    </div>
  );
